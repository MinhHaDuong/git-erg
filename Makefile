...
TEST_SUITES := validate check checkstore list ready update close migrate nextid log label new init install erg_github spec integration main archive rm datasafety security pipeline help version hook godoc docs contract roundtrip verify stderr install-staleness encoding unknown_flags selfcoherence gofmt determinism strictwrite scopeconfinement assetinvariant
