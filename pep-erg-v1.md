...
(`close`/`tag`/`untag`), swept corpus-wide by `erg migrate`, and surfaced as a
...
### 12. Tag header: a closed enum for workflow labels

<!-- GENERATED by Claude — human review needed -->

A `Tag:` header was added in v1 after initial deployment. Unlike a free-form label, it uses a closed enum for the same reasons as the header set itself: open labels proliferate without a schema.

`Tag` is repeatable because the allowed values are orthogonal flags — a ticket can legitimately carry more than one tag simultaneously. The default vocabulary is `needs-human` and `deferred`. Extended values can be configured via `tickets/.ergrc [tags]`.

The addition is backward-compatible: tickets without `Tag:` remain valid. No version bump was needed.
