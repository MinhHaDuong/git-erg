...
Two mechanisms exist because they serve different purposes. The `Closed:` header is the explicit author-intent signal: the agent or human who closes a ticket writes the reason directly into the file content. The path-based test is the structural/filesystem signal: it lets tooling move tickets into a `closed/` directory without editing file content at all. They are complementary, and `erg close` applies both at once: it writes the `Closed:` header *and* files the ticket under `closed/` in one step (ticket 0248), so there is no closed-but-unarchived limbo. Each signal still stands alone — a ticket relocated into `closed/` without a header edit reads closed by path, and a ticket closed by a hand-edited `Closed:` line reads closed by header until `erg archive` sweeps it into `closed/`. Either alone is sufficient to mark a ticket done.
...
**The contract hierarchy: seven invariants, one above the rest.**
The operating contract is seven invariants in a hierarchy: *never lose
data* is the tool's job — the reason it exists; the six traits
(agnostic, offline, standalone, fast, small, stateless) guard its form
— the shape a trustworthy data-custody tool must hold. None is a
lesser "nice to have". The hierarchy drives language and architecture
decisions: an option that fights *never lose data* is disqualified,
not negotiated. The authoritative operating statement is in
`AGENTS.md`; this PEP records the rationale that follows from it.

**Why Go over Rust/C, revisited against the six traits (2026-05-29):**
The six traits are the form-guards of the hierarchy above. The
contract is *agnostic, offline, standalone, fast, small, stateless*.
...
### 15. Superseded-by: durable supersession lineage in the closed ticket

<!-- GENERATED by Claude — human review needed -->

A `Superseded-by:` header was added after the 0221 audit, which found 47 verified supersession events across 3 stores (aedist 18, chemin-de-voix 26, git-erg 3). Cross-project recurrence proved the relation wants first-class tooling support rather than ad-hoc log-line prose. The design choices follow the precedent set by `Label` (§12) and the closed-header-set rationale (§2).

**Direction — the CLOSED ticket carries it.** The header lives on the old ticket and points at the ticket(s) that replace it; there is no `Supersedes:` header on the new ticket. The audit settled the direction empirically: 46 of the 47 events recorded the relation as "superseded by NNNN" in the *closed* ticket, against a single wild instance of the inverse (`Supersedes:` on the new ticket, aedist 0292). The durable record lives where the sweep found it.

**Repeatable.** One-to-many and many-to-one supersession are both common (0153 → 0199+0200; 24 LoRA tickets → 0237+0238 in one mass closure). Like `Label` and `Blocked-by`, the header is repeatable — one line per replacement.

**No version bump.** This is a backward-compatible addition to the closed header set (`v1HeaderKeys`); tickets without the header remain valid `%erg 0.1`. Same precedent as `Label` (§12), which was likewise added after initial deployment with no bump. The PR #385-era rejection of open `X-` extension headers (§2) is not in tension: the sanctioned path for a *new* relation is to add it to the closed set, which is exactly what this does.

**No migrate retrofit.** Historical "superseded by NNNN" log lines are durable record; rewriting them into headers would alter the historical account. `erg migrate` leaves them untouched. The header is for new supersessions going forward.

**Durable lineage — close does not strip it.** Unlike `Blocked-by` (which `erg close` removes from dependents once the blocker closes, because the dependency is discharged), supersession is a permanent historical fact. The `Superseded-by` line stays after both sides are closed and survives archival. `erg archive` needs no new guard: the header is internal to the old (closed) ticket and points *outward* at replacements, so no open ticket references the carrier through it.

**Validation tiers.** An unknown local ref is a corpus error (the referenced ID must exist, like `Blocked-by`); self-reference is an error (caught at parse time); an open ticket carrying the header is a non-fatal warning (the transition window between filing the successor and closing the old ticket is legitimate). No cycle-detection machinery is added — supersession DAGs are acyclic by construction, since the superseded ticket is closed before the header is added; a real cycle has never been observed and the machinery is deferred until one is.
