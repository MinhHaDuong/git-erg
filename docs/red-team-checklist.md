...
- **S2.2 No network code.** `tests/test_sync.sh` asserts the source carries
  ...
  only). *Expected:* pass.
- **S2.3 Option-shaped source refused.** `tests/test_sync.sh` sets
  `ERG_UPDATE_URL` and a committed `tickets/.ergrc [update] url` to
  `--upload-pack=touch MARKER`: no marker appears, the binary is untouched,
  exit 1, and the string is never echoed (ticket 0302; the fetch and
  `ls-remote` argv also carry `--end-of-options`). *Expected:* pass.
...
| S2.3 Option-shaped source refused | PASS | `tests/test_sync.sh`: "option-shaped source (env) is refused", "option-shaped source (cfg) is refused" (2026-09-17) |
