...
| Update integrity via git transport — no `net/http`, no `crypto/tls` in source; refuses cwd-repo hijack; offline no-op; refuses an option-shaped source (`--upload-pack=CMD` from the environment or a committed `.ergrc`) | shipped | tickets 0148, 0302; `tests/test_sync.sh` |
